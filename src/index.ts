...
export async function about(): Promise<void> {
  await eda.sys_Dialog.showInformationMessage(
    [
      ...
      '',
      'v0.9 Layout Preview / 执行边界：',
      '• 主流程集中在持久化 Workbench；Constraint 先生成冻结 LayoutPlan，再以 Ghost Preview 展示；',
      '• Ghost Preview 与“接受方案”都不会修改 PCB；只有物理预检通过后的 Apply 才写入；',
      '• Apply 必须消费用户刚刚接受的同一份 LayoutPlan 坐标，不能在执行阶段偷偷换位置；',
      ...
      '• Undo 不会覆盖工程师后续的手工移动。',
      '',
      '这是一条可审计、可预览、可验证、可回退的 Local Placement MVP；尚不等同于生产级全局布局优化器。',
    ].join('\n'),
    '关于 LayoutPilot',
  );
}
